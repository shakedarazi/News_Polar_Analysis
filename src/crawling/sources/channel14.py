...
from src.crawling.extractors import extract_article_paragraphs, extract_json_ld_news_article
...
class Channel14Crawler(BaseCrawler):
    ...
    def extract_article(self, url: str) -> dict[str, str]:
        html = fetch_html(url)
        title, text = extract_json_ld_news_article(html, min_len=100)
        if len(text) < 100:
            title, text = extract_article_paragraphs(html, ["article p", ".entry-content p"])
        if len(text) < 100:
