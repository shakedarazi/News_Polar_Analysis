...
export function CommentsList({ comments }: { comments: CommentItem[] }) {
  if (!comments.length) {
    return <div className="card p-8 text-center text-slate-500">אין תגובות לכתבה זו</div>;
  }
    ...
    <div className="space-y-3">
      {comments.map((c) => (
        <article key={c.comment_id} className="card p-4">
          ...
          <p className="text-sm leading-relaxed text-slate-700 whitespace-pre-wrap">{c.text}</p>
        </article>
      ))}
    </div>
  );
