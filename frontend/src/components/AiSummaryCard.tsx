  ...
  return (
    <section className="card p-6">
      <div className="mb-1 flex items-center gap-2">
        ...
        <h2 className="text-lg font-semibold text-slate-900">סיכום AI</h2>
      </div>
      ...
      {state.kind === "no-content" && (
        <p className="text-sm text-slate-500">אין מספיק תוכן בכתבה כדי לייצר סיכום.</p>
      )}
      ...
      {state.kind === "ready" && (
        <div className="space-y-4 text-sm text-slate-700">
          ...
          {!!state.data.key_points?.length && (
            <div>
              <h3 className="mb-1 text-xs font-semibold text-slate-500">נקודות מרכזיות</h3>
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500">
            {state.data.topic && (
              <span>
                <strong className="text-slate-700">נושא מרכזי:</strong> {state.data.topic}
              </span>
            )}
            {state.data.sentiment && (
              <span>
                <strong className="text-slate-700">טון הכתבה:</strong> {state.data.sentiment}
              </span>
            )}
          </div>
